          ...
          {correct ? (
            <>
              ...
              <h2 className="text-3xl font-black text-white tracking-tight">
                {hintsRevealed} Clue{hintsRevealed !== 1 ? 's' : ''} Used
              </h2>
            </>
          ) : (
            <>
          ...
          {correct && (
            <div className="flex items-center gap-4 p-3.5 rounded-xl"
              ...
              <div>
                ...
                <p className="text-2xl font-black" style={{ color: '#FFB800' }}>+{xpEarned}</p>
              </div>
              <div className="w-px h-10 bg-white/10" />
              <div>
                <p className="text-xs text-white/35 uppercase tracking-wider font-bold mb-0.5">Clues Used</p>
        ...
        <motion.div variants={item} className="mb-8 p-4 rounded-2xl"
          ...
          <div className="grid grid-cols-7 gap-1">
            {XP_TABLE.slice(1).map((xp, i) => {
              const active = hintsRevealed === i + 1 && correct;
              return (
                <div key={i} className="text-center p-1.5 rounded-lg"
                  ...
                  <p className="text-[10px] font-bold mb-0.5"
                    style={{ color: active ? '#FFB800' : 'rgba(255,255,255,0.2)' }}>
                    {i + 1}
                  </p>
                  <p className="text-[10px] font-black leading-none"
                    style={{ color: active ? '#FFD000' : 'rgba(255,255,255,0.2)' }}>
